...
class AAConsentRepository(BaseRepository[AAConsent]):
    ...
    async def update_status(
        ...
        stmt = update(AAConsent).where(AAConsent.id == consent_id_pk).values(**values)
...
class AALinkedAccountRepository(BaseRepository[AALinkedAccount]):
    ...
    async def upsert_account(
        ...
        account = AALinkedAccount(
            ...
            fip_id=fip_id, account_ref_number=account_ref, **defaults,
        )
        return await self.create(account)

    async def update_balance(self, account_id: uuid.UUID, balance: float) -> None:
        stmt = (
            update(AALinkedAccount)
            .where(AALinkedAccount.id == account_id)
            .values(current_balance=balance, last_fetched_at=datetime.utcnow())
        )
        await self.session.execute(stmt)
        await self.session.flush()
...
class AAFetchedDataRepository(BaseRepository[AAFetchedData]):
    ...
    async def list_for_user(
        self, user_id: uuid.UUID, fi_type: str | None = None,
    ) -> list[AAFetchedData]:
        stmt = (
