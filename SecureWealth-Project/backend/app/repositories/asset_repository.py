...
class PhysicalAssetRepository(BaseRepository[PhysicalAsset]):
    ...
    async def get_user_asset(self, user_id: uuid.UUID, asset_id: uuid.UUID) -> PhysicalAsset | None:
        stmt = (
    ...
    async def soft_retire(self, asset_id: uuid.UUID) -> None:
        stmt = update(PhysicalAsset).where(PhysicalAsset.id == asset_id).values(is_current=False)
        await self.session.execute(stmt)
        await self.session.flush()

    async def total_effective_value_for_user(self, user_id: uuid.UUID) -> Decimal:
        assets = await self.list_for_user(user_id, current_only=True, include_in_networth=True)
    ...
    async def summary_by_category(self, user_id: uuid.UUID) -> dict[AssetCategory, dict]:
        assets = await self.list_for_user(user_id, current_only=True, include_in_networth=True)
...
class NetWorthSnapshotRepository(BaseRepository[NetWorthSnapshot]):
    ...
    async def history_for_user(self, user_id: uuid.UUID, limit: int = 12) -> list[NetWorthSnapshot]:
        stmt = (
            select(NetWorthSnapshot)
            .where(NetWorthSnapshot.user_id == user_id)
            .order_by(NetWorthSnapshot.computed_at.desc())
