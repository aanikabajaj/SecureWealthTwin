...
class ConsentStatus(str, enum.Enum):
    """Lifecycle of an AA consent artefact."""
    PENDING   = "pending"    # Raised, awaiting user approval on AA app
    ACTIVE    = "active"     # User approved; data fetch is allowed
    PAUSED    = "paused"     # Temporarily suspended by user
    REVOKED   = "revoked"    # User revoked consent
    EXPIRED   = "expired"    # Past consent expiry date
    FAILED    = "failed"     # AA/FIP rejected the request


class FetchStatus(str, enum.Enum):
    """Status of a data-fetch session against an FIP."""
    INITIATED  = "initiated"
    IN_PROGRESS = "in_progress"
    SUCCESS    = "success"
    PARTIAL    = "partial"   # Some FIPs responded, others timed out
    FAILED     = "failed"


class AccountType(str, enum.Enum):
    """Type of financial account at the FIP."""
    SAVINGS         = "savings"
    CURRENT         = "current"
    RECURRING       = "recurring"
...
class AAConsent(Base):
    """
    Consent artefact raised to the Account Aggregator (e.g. Finvu, OneMoney).

    One user may have multiple active consents (e.g. one per FIP / purpose).
    The consent_handle is the AA-issued reference for tracking status.
    """
    ...
    # ── AA identifiers ────────────────────────────────────────────
    aa_id: Mapped[str] = mapped_column(
        String(64), nullable=False
    )
    # e.g. "finvu", "onemoney", "anumati"

    consent_handle: Mapped[str | None] = mapped_column(
        String(255), unique=True, nullable=True
    )
    # Issued by AA after consent request; used to poll status

    consent_id: Mapped[str | None] = mapped_column(
        String(255), unique=True, nullable=True
    )
    # Finalised consent ID (available only after user approval)

    # ── Consent parameters ────────────────────────────────────────
    purpose_code: Mapped[str] = mapped_column(
        String(32), nullable=False, default="03"
    )
    # RBI purpose codes: 01=Wealth Mgt, 02=Tax, 03=Financial Planning, etc.

    fi_types: Mapped[list | None] = mapped_column(JSON, nullable=True)
    # e.g. ["DEPOSIT", "MUTUAL_FUNDS", "EQUITIES"]

    date_range_from: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    date_range_to: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    consent_expiry: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    ...
    # ── Raw AA response payload (for auditability) ────────────────
    ...
    __table_args__ = (
        Index("ix_aa_consent_user_status", "user_id", "status"),
    )
...
class AALinkedAccount(Base):
    """
    A bank / FIP account discovered and linked via the AA framework.

    Each linked account maps to one FIP (Financial Information Provider)
    and one account at that institution.
    """
    ...
    # e.g. "HDFC-FIP", "SBI-FIP", "ICICI-FIP"
    ...
    # Masked/tokenised account reference from the FIP
    ...
    # ── Computed balance (updated on each fetch) ──────────────────
    ...
    __table_args__ = (
        Index("ix_aa_linked_user_fip", "user_id", "fip_id"),
    )
...
class AAFetchedData(Base):
    """
    Raw financial information packet fetched from an FIP via AA.

    Each fetch session produces one row per FIP per data type.
    Data is encrypted at rest using Fernet (KMS).
    """
    ...
    # AA-issued session/data-request ID

    fi_type: Mapped[str] = mapped_column(String(32), nullable=False)
    # "DEPOSIT", "MUTUAL_FUNDS", "EQUITIES", etc.

    status: Mapped[FetchStatus] = mapped_column(
        Enum(FetchStatus, name="fetch_status", native_enum=False),
        default=FetchStatus.INITIATED,
        nullable=False,
    )

    # ── Encrypted payload ─────────────────────────────────────────
    encrypted_payload: Mapped[str | None] = mapped_column(Text, nullable=True)
    # Fernet-encrypted JSON string of the decrypted AA data packet

    # ── Parsed summary (stored in plain JSON for querying) ────────
    summary: Mapped[dict | None] = mapped_column(JSON, nullable=True)
    # e.g. {"balance": 125000, "transactions_count": 42, "period": "2024-01"}
